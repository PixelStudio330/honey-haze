...
// Map Fix: Forces map to stay in sync
...
export default function OrderTracker({ 
  ...
  // 2. The Movement Engine (Internal State Only)
  useEffect(() => {
  ...
  // 3. Parent Sync (The fix for the Console Error)
  // We use a separate useEffect to notify the parent AFTER the render is done
  useEffect(() => {
    if (onProgressUpdate) {
  ...
  // 4. Calculate Rider Position
  const splitIndex = useMemo(() => {
    if (route.length === 0) return 0;
    // Ensure we map 0-100% to the array length correctly
    const index = Math.floor(((route.length - 1) * progress) / 100);
    return Math.min(index, route.length - 1);
  }, [route, progress]);
        ...
        <MapContainer center={SHOP_START} zoom={15} style={{ height: "100%", width: "100%" }}>
          ...
          {route.length > 0 && (
            <>
              ...
              {/* Path Sagor has already traveled (Green) */}
              <Polyline 
                positions={route.slice(0, splitIndex + 1)} 
                pathOptions={{ color: '#90be6d', weight: 5, lineCap: 'round' }} 
              />
              
              {/* Path Sagor still needs to go (Dashed Brown) */}
              <Polyline 
                positions={route.slice(splitIndex)} 
                pathOptions={{ color: '#8b5a2b', weight: 2, dashArray: '5, 10', opacity: 0.3 }} 
              />
            </>
          )}
          ...
          {/* Keyed marker to force update on every position change */}
          {riderIcon && (
            <Marker 
              key={`rider-pos-${splitIndex}`} 
              position={currentPos} 
              icon={riderIcon} 
            />
          )}
      ...
      <div className="bg-white border-2 border-[#8b5a2b] p-4 rounded-2xl shadow-[4px_4px_0px_rgba(139,90,43,0.1)]">
        <div className="flex justify-between items-end mb-2">
          <div className="flex flex-col">
             ...
             <span className="text-[9px] font-bold text-[#8b5a2b]/60">
               {progress >= 100 ? "Sagor is outside! 🍩" : "Sagor is navigating traffic..."}
             </span>
          </div>
